...
parser.add_argument('--test_dir', dest='test_dir', default='./TestingData', help='test sample are saved here')
parser.add_argument('--L1_lambda', dest='L1_lambda', type=float, default=100.0, help='weight on L1 term in objective')
parser.add_argument('--destdr', dest='destdr', help='sample output folder')
...
def main(_):
    ...
    with tf.Session(config=config) as sess:
        model = pix2pix(sess, image_size=args.fine_size, batch_size=args.batch_size, sample_size=args.sample_size,
                        ...
                        checkpoint_dir=args.checkpoint_dir, sample_dir=args.sample_dir, destdr=args.destdr)
