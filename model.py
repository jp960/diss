...
class pix2pix(object):
    ...
    def load_random_samples(self):
        preprocessed_data = np.random.choice(glob('./{0}/preprocessed/*.png'.format(self.dataset_name)),
                                             self.sample_size)
    ...
    def train(self, args):
        ...
        for epoch in xrange(args.epoch):
            ...
            data_depth = sorted(glob('./{0}/depths/*.png'.format(self.dataset_name)))
            ...
            for idx in xrange(0, batch_idxs):
                ...
                if np.mod(counter, 120) == 1:  # change back to 100
    ...
    def test(self, args):
        ...
        preprocessed_data = sorted(glob('./{0}/preprocessed/*.png'.format(args.test_dir)))
        depth_data = [path.replace('preprocessed', 'depths') for path in preprocessed_data]
        depth_images_raw = [load_image(path) for path in depth_data]
        data = list(zip(preprocessed_data, depth_data))
        sample = [load_data(sample_file[0], sample_file[1]) for sample_file in data]
        ...
        sample_depth_images = [depth_images[i:i + self.batch_size]
                               for i in xrange(0, len(depth_images), self.batch_size)]
        sample_images = np.array(sample_images)
        sample_depth_images = np.array(sample_depth_images)
        ...
        for i, sample_image in enumerate(sample_images):
            ...
            save_images(samples, sample_depth_images[i], [self.batch_size, 1], idx, g_loss, self.destdr)
